...
unionfind_fully_fused :: proc(uf: ^UnionFind) -> bool {

	for s in uf.size {
		if s != 0 {
			return s == i32(len(uf.parent))
		}
	}
...
main :: proc() {
	...
	for pair in g_pairs {
		unionfind_union(&uf_set, pair.point_a, pair.point_b)

		if unionfind_fully_fused(&uf_set) {
			fmt.println(g_points[pair.point_a][0] * g_points[pair.point_b][0])
			break
		}
	}
}
